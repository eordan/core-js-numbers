...
function isPrime(n) {
  return n <= 1
    ? false
    : Array.from({ length: Math.floor(Math.sqrt(n)) }, (e, i) => i + 2).every(
        (curr) => (n === 2 ? true : n % curr !== 0)
      );
}
...
function getFibonacciNumber(index) {
  return Array.from({ length: index + 1 }).reduce(
...
function getSumToN(n) {
  return Array.from({ length: n }, (e, i) => i + 1).reduce(
    (acc, curr) => acc + curr
  );
}
